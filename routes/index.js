...
router.post(
  "/user",
  [check("uid").notEmpty().withMessage("must have uid").trim().escape()],
...
router.post(
  "/isSaved",
  [
    check("uid", "empty uid").not().isEmpty().trim().escape(),
    check("fid").isInt().withMessage("fact id must be an integer"),
  ],
  auth.checkIfAuthenticated,
  db.checkSaved
);
...
*/
router.post("/isAdmin", auth.checkIfAdmin, function (req, res, err) {
  res.status(200);
  res.send({ r: true });
});
...
router.post(
  "/addUser",
  [
    check("uid", "must have uid").not().isEmpty().trim().escape(),
    check("email")
      .normalizeEmail()
      .isEmail()
...
router.post(
  ...
  [
    ...
    check("issue")
      .notEmpty()
      .withMessage("issue cannot be empty")
      .trim()
      .escape(),
  ],
  auth.checkIfAuthenticated,
